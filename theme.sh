...
# --- The speed dial on the new tab page.
#
# Issue 9 asked for a new tab page whose shortcuts the user arranges, rather
# than a grid derived from history: "a 4x4 layout and shortcut buttons", with
# the complaint about Cromite being that its tiles come from browsing history
# and cannot be edited.
#
# Chromium has its own answer to that and it is switched on in this build:
# MostVisitedSites carries a full custom-links API - AddCustomLinkTo,
# UpdateCustomLink, ReorderCustomLink, DeleteCustomLink - the Android bridge
# exposes every one of them over JNI, and MostVisitedSitesBridge.java gates
# them on ChromeFeatureList.sMostVisitedTilesCustomization, which is
# FEATURE_ENABLED_BY_DEFAULT at 152 and which neither this script nor Vanadium
# turns off. That was checked rather than assumed, and it is written down here
# because it is the reason a reviewer might ask why this file exists at all.
#
# It exists because building on it was considered and not chosen: the decision
# was to own the surface rather than dress upstream's. The cost of that is
# stated plainly - this duplicates a feature Chromium already ships, and the
# new tab page is a surface Chromium reworks most milestones.
#
# What limits the damage is that AeriumSpeedDial.java touches nothing in the
# suggestions or tiles stack. It reads one shared-preference string, draws
# framework views, and opens a URL with an ordinary VIEW intent aimed at this
# package. Upstream's tiles are left fully wired and simply hidden, so the
# coordinator that drives them keeps finding the views it expects and cannot
# crash on a container that changed shape underneath it. Removing all of this
# later is deleting one file and three substitutions.
AERIUM_SD=chrome/android/java/src/org/chromium/chrome/browser/ntp/AeriumSpeedDial.java
cat > $AERIUM_SD <<'AERIUM_SPEED_DIAL_JAVA'
// Copyright 2026 The Aerium Authors
// Aerium: generated by theme.sh. See that script for why this exists.
...
/**
 * Aerium: the speed dial on the new tab page - a grid of shortcuts the user arranges, in place of
 * Chromium's history-derived Most Visited tiles.
 *
 * <p>Deliberately self-contained. It reads and writes one shared-preference string, draws itself
 * out of plain framework views, and opens a URL with an ordinary VIEW intent aimed at this package.
 * Nothing here reaches into the suggestions or tiles stack, so a Chromium reshuffle of those - which
 * happens most milestones - cannot break it, and this file can be deleted in one piece.
 *
 * <p>Backgrounds are drawn with GradientDrawable rather than drawable XML on purpose: a resource
 * file is one more thing the build has to be told about, and an outline with a corner radius is two
 * lines of Java.
 */
public class AeriumSpeedDial extends LinearLayout {
    /** One shortcut. */
    ...
    public AeriumSpeedDial(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private static int maxTiles() {
        int rows =
                ChromeSharedPreferences.getInstance()
                        .readInt(ChromePreferenceKeys.AERIUM_SPEED_DIAL_ROWS, 4);
        if (rows < 1) rows = 1;
        if (rows > 6) rows = 6;
        return columns() * rows;
    }
    ...
    private static List<Entry> readEntries() {
        ...
        String raw =
                ChromeSharedPreferences.getInstance()
                        .readString(ChromePreferenceKeys.AERIUM_SPEED_DIAL_TILES, "");
        ...
        try {
            ...
            for (int i = 0; i < array.length(); i++) {
                ...
                String url = item.optString(KEY_URL, "");
                if (TextUtils.isEmpty(url)) continue;
                out.add(new Entry(item.optString(KEY_TITLE, ""), url));
            }
        } catch (JSONException e) {
            // A hand-edited or truncated value reads as no shortcuts rather than as a crash. The
    ...
    private static void writeEntries(List<Entry> entries) {
        ...
        for (int i = 0; i < entries.size(); i++) {
            ...
            try {
                item.put(KEY_TITLE, entries.get(i).title);
                item.put(KEY_URL, entries.get(i).url);
            } catch (JSONException e) {
                continue;
            }
            array.put(item);
        ...
        ChromeSharedPreferences.getInstance()
                .writeString(ChromePreferenceKeys.AERIUM_SPEED_DIAL_TILES, array.toString());
    }
    ...
    private void rebuild() {
        ...
        for (int i = 0; i < shown; i++) {
            grid.addView(buildTile(entries.get(i), i));
        }
        if (shown < limit) {
    ...
    private GradientDrawable tileBackground(boolean dashed) {
        GradientDrawable shape = new GradientDrawable();
        shape.setShape(GradientDrawable.RECTANGLE);
    ...
    private TextView buildLabel(String text, int color) {
    ...
    private LinearLayout buildCell() {
    ...
    private View buildTile(final Entry entry, final int index) {
        LinearLayout cell = buildCell();

        TextView face = new TextView(getContext());
        face.setBackground(tileBackground(false));
        face.setGravity(Gravity.CENTER);
        face.setTextColor(ACCENT);
        face.setTextSize(21);
        face.setText(monogram(entry));
        int side = dp(56);
        // FrameLayout params, because face goes into the stack below rather than straight into
        // the cell. FrameLayout would convert LinearLayout params rather than reject them, but
        // the converted copy drops gravity, and the badge beside it depends on gravity working.
        face.setLayoutParams(new FrameLayout.LayoutParams(side, side));

        FrameLayout stack = new FrameLayout(getContext());
        stack.setLayoutParams(
        ...
        if (mEditing) {
            ...
            remove.setText("×");
            remove.setTextColor(0xFF06283D);
            ...
            remove.setLayoutParams(badgeParams);
    ...
    private static String displayTitle(Entry entry) {
        if (!TextUtils.isEmpty(entry.title)) return entry.title;
        String host = hostOf(entry.url);
        return TextUtils.isEmpty(host) ? entry.url : host;
    }

    private static String monogram(Entry entry) {
        String source = !TextUtils.isEmpty(entry.title) ? entry.title : hostOf(entry.url);
    ...
    /**
     * Aerium: normalises what the user typed. A bare "lwn.net" becomes https, and anything that is
     * not http or https is refused - a shortcut is a web address, and an intent aimed at this
     * package with some other scheme is a way to reach places the address bar would not.
     */
    private static String normalizeUrl(String typed) {
        ...
        if (TextUtils.isEmpty(parsed.getHost())) return "";
        return trimmed;
    }

    private void open(String url) {
        String safe = normalizeUrl(url);
        if (TextUtils.isEmpty(safe)) return;
        ...
        try {
            ...
            // Nothing to do: the shortcut simply does not open.
        }
    }
    ...
    private void showAddDialog() {
        ...
        nameField.setHint(R.string.aerium_speed_dial_name);
        nameField.setSingleLine(true);

        final EditText urlField = new EditText(context);
...
AERIUM_SPEED_DIAL_JAVA

# chrome_java_sources.gni enumerates the Java it compiles rather than globbing
# the tree, so a file that is not named here is not built and R8 never sees it.
sed_i 's|^  "java/src/org/chromium/chrome/browser/ntp/NewTabPageLayout.java",$|  "java/src/org/chromium/chrome/browser/ntp/AeriumSpeedDial.java",\n&|' \
    chrome/android/chrome_java_sources.gni

# The keys, declared and registered the two ways ChromePreferenceKeys documents
# at the top of itself. Skipping the second step builds fine and then trips
# StrictPreferenceKeyChecker in any build with asserts on.
sed_i 's|    public static final String AERIUM_BOTTOM_BAR = "Chrome.Aerium.BottomBar";|&\n\n    /** Whether the speed dial replaces the Most Visited tiles on the new tab page. */\n    public static final String AERIUM_SPEED_DIAL = "Chrome.Aerium.SpeedDial";\n\n    /** The speed dial shortcuts, as a JSON array of {"title", "url"} objects. */\n    public static final String AERIUM_SPEED_DIAL_TILES = "Chrome.Aerium.SpeedDialTiles";\n\n    /** Speed dial shortcuts per row - 3 or 4. */\n    public static final String AERIUM_SPEED_DIAL_COLUMNS = "Chrome.Aerium.SpeedDialColumns";\n\n    /** Speed dial rows of shortcuts. */\n    public static final String AERIUM_SPEED_DIAL_ROWS = "Chrome.Aerium.SpeedDialRows";|' \
    $CPK
sed_i 's|^                AERIUM_BOTTOM_BAR,$|&\n                AERIUM_SPEED_DIAL,\n                AERIUM_SPEED_DIAL_COLUMNS,\n                AERIUM_SPEED_DIAL_ROWS,\n                AERIUM_SPEED_DIAL_TILES,|' \
    $CPK

# The view sits where the tiles sat, with the same margins, so the page keeps
# its rhythm whichever grid is showing.
sed_i 's|        android:layout="@layout/mv_tiles_layout" />|&\n\n    <!-- Aerium: the speed dial. See theme.sh. -->\n    <org.chromium.chrome.browser.ntp.AeriumSpeedDial\n        android:id="@+id/aerium_speed_dial"\n        android:layout_width="match_parent"\n        android:layout_height="wrap_content"\n        android:layout_marginTop="@dimen/ntp_section_top_margin"\n        android:layout_marginBottom="@dimen/ntp_section_bottom_margin"\n        android:layout_marginLeft="@dimen/mvt_container_lateral_margin"\n        android:layout_marginRight="@dimen/mvt_container_lateral_margin"\n        android:orientation="vertical" />|' \
    chrome/android/java/res/layout/new_tab_page_layout.xml

# Upstream's tiles are hidden rather than not inflated. initializeSiteSectionView
